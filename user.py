...
def u_log():
    ...
    if hash_pw == result[0]["pw"]:   
        ...
        s_flag = result[0]["herb"]
        session["s_flag"] = s_flag
        
        if s_flag == 0:
            return redirect("/tutorial_story")
        else :
            return redirect("/gametop")
...
def main_map():
    ...
    dbmg = db_manager()
    result = dbmg.exec_query("select * from user where id = %s",user_id)
    life = result[0]["blessing1"]
    session["life"] = life + 3
    flag = result[0]["blessing3"]
    session["flag"] = flag
    
    return render_template("/user/main_map.html",flag=flag)
...
def main_map_2():
    ...
    return render_template("/user/main_map_2.html",flag=flag)
# -----------------------------------------------
...
def main_quiz():
    ...
    try:
        ...
        if life <= 0:
            ...
            return render_template("/user/main_map.html",hantei=hantei,flag=flag)
        # 3問目だったら終了
        if count > 3:
            result = dbmg.exec_query("select * from user where id = %s",user_id)
            flag = 1
            moto_flag = session["flag"]
            if flag > moto_flag :
                sql = "UPDATE user SET blessing3 =%s WHERE id = %s"
                dbmg.exec_query(sql,(flag,user_id))
            else :
                flag = moto_flag

            # 経験値追加
            keikenti = result[0]["blessing2"] + 100   #←←←←←←このステージの経験値
            sql = "UPDATE user SET blessing2 =%s WHERE id = %s"
            dbmg.exec_query(sql,(keikenti,user_id))
            hantei = "クリア！これからも頑張ろう！！！経験値100獲得"
        ...
        result=result,quiz_id=quiz_id,new_str=new_str,life=life)

    except IndexError as e:
        session["quiz_count"] = session["quiz_count"]- 1
        return render_template("/user/err.html")
    ...
    try:
        ...
        if count > 4:
            ...
            if flag > moto_flag :
                ...
                flag = moto_flag
            # 経験値追加
            keikenti = result[0]["blessing2"] + 200   #←←←←←←このステージの経験値
            sql = "UPDATE user SET blessing2 =%s WHERE id = %s"
            dbmg.exec_query(sql,(keikenti,user_id))
            hantei = "クリア！これからも頑張ろう！！！経験値200獲得"
        ...
        return render_template("/user/main_quiz2.html",
        result=result,quiz_id=quiz_id,new_str=new_str,life=life)
...
# main問題画面 処理 (回答)
@app.route("/kaitou_quiz_main2")
def kaitou_quiz_main2():
        quiz_id = request.args.get("quiz_id")
        user_id = session["user_id"]
        life = session["life"]
        user_answer = request.args.get("q1")
        ...
        if int(user_answer) == answer:
            ...
            session["life"] = life
            # dbmg.exec_query("insert into review value(%s,%s)",(user_id,quiz_id))
        return render_template("/user/main_right2.html",ox=ox,user_id=user_id,quiz=quiz)
# -------------------------------------------------
# 解説画面 画像表示
@app.route("/main_quiz_right2", methods=["POST"])
def main_quiz_right2():
    dbmg = db_manager()
    ...
    if quiz_id == quiz[0]["comment"]:
        ...
        # 文字付けたし
        new_str = "../static/images/Answer" + new_str
        return render_template("/user/main_right2.html",new_str=new_str,user_id=user_id,quiz_id=quiz_id,quiz=quiz)
    else:
        return render_template("test.html")
...
def main_quiz3():
    ...
    if count > 5:
        ...
        # 経験値追加
        keikenti = result[0]["blessing2"] + 300   #←←←←←←このステージの経験値
        sql = "UPDATE user SET blessing2 =%s WHERE id = %s"
        dbmg.exec_query(sql,(keikenti,user_id))
        hantei = "クリア！これからも頑張ろう！！！経験値300獲得"
        ...
        flag = result[0]["blessing3"]
        session["flag"] = flag
        return render_template("/user/main_map.html",hantei=hantei)
    else:
        session['quiz_count'] = count
    ...
    life = session["life"]
    return render_template("/user/main_quiz3.html",
    result=result,quiz_id=quiz_id,new_str=new_str,life=life)

# main問題画面 処理 (回答)
@app.route("/kaitou_quiz_main3")
def kaitou_quiz_main3():
        quiz_id = request.args.get("quiz_id")
        user_id = session["user_id"]
        life = session["life"]
        user_answer = request.args.get("q1")
        ...
        if int(user_answer) == answer:
            ...
            session["life"] = life
            # dbmg.exec_query("insert into review value(%s,%s)",(user_id,quiz_id))
        return render_template("/user/main_right3.html",ox=ox,user_id=user_id,quiz=quiz)
# -------------------------------------------------
# メインクイズ問題画面 表示 4
@app.route("/main_quiz4")
def main_quiz4():
    dbmg = db_manager()
    user_id = session["user_id"]
    life = session["life"]
    ...
    if count > 6:
        ...
        # 経験値追加
        keikenti = result[0]["blessing2"] + 400   #←←←←←←このステージの経験値
        sql = "UPDATE user SET blessing2 =%s WHERE id = %s"
        dbmg.exec_query(sql,(keikenti,user_id))
        hantei = "クリア！これからも頑張ろう！！！経験値400獲得"
    ...
    return render_template("/user/main_quiz4.html",
    result=result,quiz_id=quiz_id,new_str=new_str,life=life)

# main問題画面 処理 (回答)
@app.route("/kaitou_quiz_main4")
def kaitou_quiz_main4():
        quiz_id = request.args.get("quiz_id")
        user_id = session["user_id"]
        life = session["life"]
        user_answer = request.args.get("q1")
        ...
        if int(user_answer) == answer:
            ...
            session["life"] = life
            # dbmg.exec_query("insert into review value(%s,%s)",(user_id,quiz_id))
        return render_template("/user/main_right4.html",ox=ox,user_id=user_id,quiz=quiz)

# -------------------------------------------------
# メインクイズ問題画面 表示 5
@app.route("/main_quiz5")
def main_quiz5():
    dbmg = db_manager()
    user_id = session["user_id"]
    life = session["life"]
    ...
    if count > 7:
        ...
        # 経験値追加
        keikenti = result[0]["blessing2"] + 500   #←←←←←←このステージの経験値
        sql = "UPDATE user SET blessing2 =%s WHERE id = %s"
        dbmg.exec_query(sql,(keikenti,user_id))
        hantei = "クリア！これからも頑張ろう！！！経験値500獲得"
    ...
    return render_template("/user/main_quiz5.html",
    result=result,quiz_id=quiz_id,new_str=new_str,life=life)

# main問題画面 処理 (回答)
@app.route("/kaitou_quiz_main5")
def kaitou_quiz_main5():
        quiz_id = request.args.get("quiz_id")
        user_id = session["user_id"]
        life = session["life"]
        user_answer = request.args.get("q1")
        ...
        if int(user_answer) == answer:
            ...
            session["life"] = life
            # dbmg.exec_query("insert into review value(%s,%s)",(user_id,quiz_id))
        return render_template("/user/main_right5.html",ox=ox,user_id=user_id,quiz=quiz)

# -------------------------------------------------
# メインクイズ問題画面 表示 6
@app.route("/main_quiz6")
def main_quiz6():
    dbmg = db_manager()
    user_id = session["user_id"]
    life = session["life"]
    ...
    if count > 8:
        ...
        if flag >= moto_flag :
            ...
            flag = moto_flag
        # 経験値追加
        keikenti = result[0]["blessing2"] + 600   #←←←←←←このステージの経験値
        sql = "UPDATE user SET blessing2 =%s WHERE id = %s"
        dbmg.exec_query(sql,(keikenti,user_id))
        hantei = "クリア！これからも頑張ろう！！！経験値600獲得"
    ...
    return render_template("/user/main_quiz6.html",
    result=result,quiz_id=quiz_id,new_str=new_str,life=life)

# main問題画面 処理 (回答)
@app.route("/kaitou_quiz_main6")
def kaitou_quiz_main6():
        quiz_id = request.args.get("quiz_id")
        user_id = session["user_id"]
        life = session["life"]
        user_answer = request.args.get("q1")
        ...
        if int(user_answer) == answer:
            ...
            session["life"] = life
            # dbmg.exec_query("insert into review value(%s,%s)",(user_id,quiz_id))
        return render_template("/user/main_right6.html",ox=ox,user_id=user_id,quiz=quiz)

# -------------------------------------------------
# メインクイズ問題画面 表示 7
@app.route("/main_quiz7")
def main_quiz7():
    dbmg = db_manager()
    user_id = session["user_id"]
    life = session["life"]
    ...
    if count > 9:
        ...
        if flag >= moto_flag :
            ...
            flag = moto_flag
        # 経験値追加
        keikenti = result[0]["blessing2"] + 700   #←←←←←←このステージの経験値
        sql = "UPDATE user SET blessing2 =%s WHERE id = %s"
        dbmg.exec_query(sql,(keikenti,user_id))
        hantei = "クリア！これからも頑張ろう！！！経験値700獲得"
    ...
    return render_template("/user/main_quiz7.html",
    result=result,quiz_id=quiz_id,new_str=new_str,life=life)

# main問題画面 処理 (回答)
@app.route("/kaitou_quiz_main7")
def kaitou_quiz_main7():
        quiz_id = request.args.get("quiz_id")
        user_id = session["user_id"]
        life = session["life"]
        user_answer = request.args.get("q1")
        ...
        if int(user_answer) == answer:
            ...
            session["life"] = life
            # dbmg.exec_query("insert into review value(%s,%s)",(user_id,quiz_id))
        return render_template("/user/main_right7.html",ox=ox,user_id=user_id,quiz=quiz)

# -------------------------------------------------
# メインクイズ問題画面 表示 8
@app.route("/main_quiz8")
def main_quiz8():
    dbmg = db_manager()
    user_id = session["user_id"]
    life = session["life"]
    ...
    if count > 10:
        ...
        if flag >= moto_flag :
            ...
            flag = moto_flag
        # 経験値追加
        keikenti = result[0]["blessing2"] + 800   #←←←←←←このステージの経験値
        sql = "UPDATE user SET blessing2 =%s WHERE id = %s"
        dbmg.exec_query(sql,(keikenti,user_id))
        hantei = "クリア！これからも頑張ろう！！！経験値800獲得"
    ...
    return render_template("/user/main_quiz8.html",
    result=result,quiz_id=quiz_id,new_str=new_str,life=life)

# main問題画面 処理 (回答)
@app.route("/kaitou_quiz_main8")
def kaitou_quiz_main8():
        quiz_id = request.args.get("quiz_id")
        user_id = session["user_id"]
        life = session["life"]
        user_answer = request.args.get("q1")
        ...
        if int(user_answer) == answer:
            ...
            session["life"] = life
            # dbmg.exec_query("insert into review value(%s,%s)",(user_id,quiz_id))
        return render_template("/user/main_right8.html",ox=ox,user_id=user_id,quiz=quiz)

# -------------------------------------------------
# メインクイズ問題画面 表示 9
@app.route("/main_quiz9")
def main_quiz9():
    dbmg = db_manager()
    user_id = session["user_id"]
    life = session["life"]
    ...
    if count > 11:
        ...
        if flag >= moto_flag :
            ...
            flag = moto_flag
        # 経験値追加
        keikenti = result[0]["blessing2"] + 900   #←←←←←←このステージの経験値
        sql = "UPDATE user SET blessing2 =%s WHERE id = %s"
        dbmg.exec_query(sql,(keikenti,user_id))
        hantei = "クリア！これからも頑張ろう！！！経験値900獲得"
    ...
    return render_template("/user/main_quiz9.html",
    result=result,quiz_id=quiz_id,new_str=new_str,life=life)

# main問題画面 処理 (回答)
@app.route("/kaitou_quiz_main9")
def kaitou_quiz_main9():
        quiz_id = request.args.get("quiz_id")
        user_id = session["user_id"]
        life = session["life"]
        user_answer = request.args.get("q1")
        ...
        if int(user_answer) == answer:
            ...
            session["life"] = life
            # dbmg.exec_query("insert into review value(%s,%s)",(user_id,quiz_id))
        return render_template("/user/main_right9.html",ox=ox,user_id=user_id,quiz=quiz)

# -------------------------------------------------
# メインクイズ問題画面 表示 10
@app.route("/main_quiz10")
def main_quiz10():
    dbmg = db_manager()
    user_id = session["user_id"]
    life = session["life"]
    ...
    if count > 12:
        ...
        if flag >= moto_flag :
            ...
            flag = moto_flag
        # 経験値追加
        keikenti = result[0]["blessing2"] + 1000   #←←←←←←このステージの経験値
        sql = "UPDATE user SET blessing2 =%s WHERE id = %s"
        dbmg.exec_query(sql,(keikenti,user_id))
    ...
    return render_template("/user/main_quiz10.html",
    result=result,quiz_id=quiz_id,new_str=new_str,life=life)

# main問題画面 処理 (回答)
@app.route("/kaitou_quiz_main10")
def kaitou_quiz_main10():
        quiz_id = request.args.get("quiz_id")
        user_id = session["user_id"]
        life = session["life"]
        user_answer = request.args.get("q1")
        ...
        if int(user_answer) == answer:
            ...
            session["life"] = life
            # dbmg.exec_query("insert into review value(%s,%s)",(user_id,quiz_id))
        return render_template("/user/main_right10.html",ox=ox,user_id=user_id,quiz=quiz)
...
def minigame_quiz_page():
    ...
    try:
        ...
        return render_template("/user/minierr.html")
...
def tutorial_story():
        now_story = request.args.get("now_story")
        if now_story == None :
            now_story = 0
        else:
            now_story = int(now_story)
        print(now_story)
        print("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        print(type("now_story"))
        
        user_id =session["user_id"]
        s_flag = session["s_flag"]
        dbmg = db_manager()
        
        if  s_flag >= now_story & s_flag >= 1 :
            return render_template("/user/main_map.html")
        elif s_flag == 0 :
            sql = "update user set herb = 1 where id = %s"
            dbmg.exec_query(sql,(user_id))
            session["s_flag"] = now_story
            return render_template("/user/tutorial_story.html",s_flag=s_flag)
        else :
            sql = "update user set herb = %s where id = %s"
            dbmg.exec_query(sql,(now_story,user_id))
            session["s_flag"] = now_story
            return render_template("/user/tutorial_story.html",s_flag=now_story)
...
def story_1():
        ...
        dbmg = db_manager()
        if s_flag <= 2 :
            sql = "update user set herb = 2 where id = %s"
            dbmg.exec_query(sql,(user_id))
            session["s_flag"] = 2
            s_flag = 2
            return render_template("/user/tutorial_story.html",s_flag=s_flag)
...
def story_5():
        ...
        dbmg = db_manager()
        if s_flag <= 5 :
            sql = "update user set herb = 5 where id = %s"
            dbmg.exec_query(sql,(user_id))
            session["s_flag"] = 5
            s_flag = 5
            return render_template("/user/tutorial_story.html",s_flag=s_flag)
        else :
            return redirect("/main_game5")
...
def story_9():
        ...
        dbmg = db_manager()
        if s_flag <= 9 :
            sql = "update user set herb = 9 where id = %s"
            dbmg.exec_query(sql,(user_id))
            session["s_flag"] = 9
            s_flag = 9
            return render_template("/user/tutorial_story.html",s_flag=s_flag)
        else :
            return redirect("/main_game10")
...
def story_10():
        ...
        dbmg = db_manager()
        if s_flag <= 10 :
            sql = "update user set herb = 10 where id = %s"
            dbmg.exec_query(sql,(user_id))
            session["s_flag"] = 10
            s_flag = 10
            return render_template("/user/tutorial_story.html",s_flag=s_flag)
...
def board():
    ...
    sql = "insert into board (thread_id,user_id,comment,date,thread_name) value (%s,%s,%s,%s,%s)"
    dbmg.exec_query(sql, (thread_id,name,comment,date,thread_name))
...
def board_page():
    ...
    return render_template("/user/board.html", comments=comments,thread_id=thread_id,user_id=user_id,thread_name=thread_name)
# 削除依頼<コメント>
...
def u_board_comdelete_b():
    ...
    comment_id = com[0]["comment_id"]
    date = com[0]["date"]
    user_id = com[0]["user_id"]
    comment = com[0]["comment"]
    thread_name = com[0]["thread_name"]
    sql = "insert into delete_board (comment_id,date,user_id,comment,thread_name,reason) value (%s,%s,%s,%s,%s,%s)"
    dbmg.exec_query(sql, (comment_id,date,user_id,comment,thread_name,reason))
...
def u_board_thrdelete_b():
    ...
    thread_id = com[0]["thread_id"]
    thread_name = com[0]["thread_name"]
    user_id = com[0]["user_id"]
    sql = "insert into delete_thread (thread_id,thread_name,user_id,reason_2) value (%s,%s,%s,%s)"
    dbmg.exec_query(sql, (thread_id,thread_name,user_id,reason_2))
    return redirect("/thread_page")
...
# -------------------------------------------------
#掲示板管理
@app.route("/thrboard_edit")
def thrboard_edit():
    return render_template("/ad/thrboard_edit.html")
#掲示板削除依頼(スレッド)
@app.route("/thread_edit")
def thread_edit_page():
    dbmg = db_manager()
    thread = dbmg.exec_query("select * from delete_thread")
    return render_template("/ad/thread_edit.html",thread=thread)
#掲示板削除依頼(コメント)
@app.route("/board_edit")
def board_edit_page():
    dbmg = db_manager()
    board = dbmg.exec_query("select * from delete_board")
    return render_template("/ad/board_edit.html",board=board)
# 掲示板削除処理<スレッド>
@app.route("/thread_edit_page_del",methods=["POST"])
def thread_edit_page_del():
    delete_thread_id=request.form.get("delete_thread_id")
    thread_id = request.form.get("thread_id")
    dbmg = db_manager()
    dbmg.exec_query("DELETE FROM delete_thread WHERE delete_thread_id = %s",delete_thread_id)
    dbmg.exec_query("DELETE FROM board WHERE comment_id = %s",thread_id)
    dbmg.exec_query("DELETE FROM thread WHERE thread_id = %s",thread_id)
    return redirect("/thrboard_edit")
# 掲示板削除処理<スレッド>
@app.route("/board_edit_page_del",methods=["POST"])
def board_edit_page_del():
    delete_comment_id=request.form.get("delete_comment_id")
    comment_id = request.form.get("comment_id")
    dbmg = db_manager()
    dbmg.exec_query("DELETE FROM delete_board WHERE delete_comment_id = %s",delete_comment_id)
    dbmg.exec_query("DELETE FROM board WHERE comment_id = %s",comment_id)
    return redirect("/thrboard_edit")
